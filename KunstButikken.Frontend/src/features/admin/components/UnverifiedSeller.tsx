...
import type { Seller } from "@/features/art/types/art";

export default function UnverifiedSeller() {
      ...
      if (res.status === 401) {
        ...
        const normalized = arr.map((it: unknown) => {
          ...
          return ({
            ...
            displayName: (obj.displayName ?? obj.name ?? obj.username ?? '(no name)') as string,
            email: (obj.email ?? obj.emailAddress ?? '') as string,
          } as Seller);
        });
        setList(normalized);
      }
  ...
  const setVerified = async (id: string, verified: boolean) => {
    try {
      ...
      showSnack(verified ? 'Marked as seller.' : 'Marked not a seller.', 'success');
    } catch (err) {
  ...
  if (!keycloak.authenticated) {
    return (
      <Box sx={{ p: 4 }}>
        <Typography variant="h6">Admin — Unverified sellers</Typography>
  ...
  return (
    <Box sx={{ p: 4 }}>
      ...
      {loading ? (
        ...
        <Alert severity="info">No unverified sellers found.</Alert>
      ) : (
        <List>
          {list.map((s) => (
            <ListItem
              ...
              secondaryAction={(
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mr: 1 }}>
                  ...
                  <IconButton edge="end" aria-label={t("aria.reject")} title={t("admin.reject")} onClick={() => openRejectDialog(s.id, s.displayName ?? '')}>
                    <CloseIcon color="error" />
                  </IconButton>
                </Box>
              )}
            >
              <ListItemText primary={s.displayName || "(no name)"} secondary={s.email} />
            </ListItem>
          ))}
        </List>
      )}
      ...
      <Dialog open={rejectDialogOpen} onClose={closeRejectDialog} aria-labelledby="reject-dialog-title">
        <DialogTitle id="reject-dialog-title">Confirm action</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to mark "{rejectTarget?.label ?? ''}" as not a seller? This action cannot be undone here.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
