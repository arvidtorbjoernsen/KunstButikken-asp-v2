...
export class HomeComponent {
  ...
  readonly featured = toSignal(this.store.featured$, { initialValue: [] });
  readonly loadingArt = toSignal(this.store.loadingArt$, { initialValue: false });
  readonly loadingFeatured = toSignal(this.store.loadingFeatured$, { initialValue: false });
  readonly error = toSignal(this.store.error$, { initialValue: undefined as string | undefined });
  readonly hasArt = computed(() => this.art().length > 0);
  ...
  // Angular 20 pattern: Load data in constructor
  constructor() {
